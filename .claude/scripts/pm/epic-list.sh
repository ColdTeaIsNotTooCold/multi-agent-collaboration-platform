...
[ ! -d ".claude/epics" ] && echo "📁 未找到史诗目录。使用以下命令创建第一个史诗: /pm:prd-parse <feature-name>" && exit 0
[ -z "$(ls -d .claude/epics/*/ 2>/dev/null)" ] && echo "📁 未找到史诗。使用以下命令创建第一个史诗: /pm:prd-parse <feature-name>" && exit 0
